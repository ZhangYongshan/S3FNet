...
def SAM(H_fuse, H_ref):
    ...
    # Rehsaping fused and reference data
    H_fuse_reshaped = H_fuse.contiguous().view(N_spectral, -1)
    H_ref_reshaped = H_ref.contiguous().view(N_spectral, -1)
...
def RMSE(H_fuse, H_ref):
    # Rehsaping fused and reference data
    H_fuse_reshaped = H_fuse.contiguous().view(-1)
    H_ref_reshaped = H_ref.contiguous().view(-1)
